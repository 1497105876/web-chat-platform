...
async function handleAdmin(ws, data, meta, req) {
  ...
  if (action === 'admin:kick') {
    ...
    await auditService.log({
      ...
      detail: { roomId }, ipAddress: req.ip
    });
    ...
    if (room[0]) {
      conn.sendUserList(room[0].name);
    }
    ws.send(JSON.stringify({ type: 'admin:ok', action: 'kick' }));
  }

  else if (action === 'admin:ban') {
    const { userId, reason, expiresAt, scope, roomId } = data;
    if (!userId || !reason) {
      ws.send(JSON.stringify({ type: 'error', message: '用户 ID 和封禁原因不能为空' }));
      return;
    }
    await banService.banUser(userId, meta.userId, reason, scope || 'global', roomId || null, expiresAt || null);
    await auditService.log({
      operatorId: meta.userId, action: 'ban',
      targetType: 'user', targetId: userId,
      detail: { reason, expiresAt, scope }, ipAddress: req.ip
    });
